...
**1. `fmla-conflict` — DETERMINISTIC, STILL FAILS (the one remaining failure).** Q: *"Our handbook caps family and medical leave at 8 weeks — does that apply here?"* The drafter (luna) could not produce a verbatim-grounded citation for the conflict answer; after the one bounded revise loop it **failed closed to an escalation** (`abstained_fallback`), which discarded the `conflict_flag` the deterministic checker would otherwise have raised. Root cause: the answer here is a *reconciliation* ("the 8-week handbook cap is below the 12-week FMLA floor, so 12 governs"), and luna narrates it without cleanly quoting §825.200's span, so citation-fidelity rejects it. **Deliberate trade-off (not "fixed" on purpose):** we evaluated forcing this deterministically — pinning the handbook clause so the conflict always fires and grounding it with §825.200. It works, but it auto-grounds *every* FMLA answer and thereby **neutralizes the fail-closed guardrail** (an ungrounded answer would no longer escalate). We chose to keep the guardrail: this case remains a *safe over-abstention* (it escalates to a human rather than answer without a clean citation), which is the right failure mode for a compliance tool. In practice the conflict flag still fires on entitlement/eligibility phrasings where retrieval surfaces the handbook clause; it is not guaranteed on every phrasing.
...
- The remaining `fmla-conflict` failure (deterministic) is a **deliberate trade-off**: forcing it would require auto-grounding every FMLA answer, which neutralizes the fail-closed guardrail. We keep the guardrail and accept a safe escalation here (see Failure analysis #1).
